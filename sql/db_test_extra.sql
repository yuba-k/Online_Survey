...
SELECT
  ...
  'JT20260702 PC Environment Survey',
    FALSE,
  '{"title":"PC usage overview","Survey_tag":["PC","work","hobby"],"aggregate":{"gender":true,"age":true,"gender_split":false},"questions":[{"id":"q1","type":"single","label":"Main OS?","options":["Windows","Mac","Linux"],"result_display":"pie"},{"id":"q2","type":"multiple","label":"Typical use cases?","options":["Work","Gaming","Video editing","Development"],"result_display":"bar"},{"id":"q3","type":"text","label":"Free text"}]}',
...
SELECT
  ...
  'JT20260702 Free Text Survey',
    TRUE,
  '{"title":"Free text focused survey","Survey_tag":["free-text","UI"],"aggregate":{"gender":false,"age":true,"gender_split":true},"questions":[{"id":"q1","type":"text","label":"What should be improved?"},{"id":"q2","type":"text","label":"What is hard to use?"}]}',
...
SELECT
    ...
    FALSE,
    '{"title":"Mixed question types","Survey_tag":["single-choice","multi-choice","text"],"aggregate":{"gender":true,"age":true,"gender_split":true},"questions":[{"id":"q1","type":"single","label":"Usage frequency","options":["Daily","Several times a week","Several times a month"],"result_display":"bar"},{"id":"q2","type":"multiple","label":"Things you care about","options":["Speed","Design","Usability","Notifications"],"result_display":"table"},{"id":"q3","type":"text","label":"Requests"}]}',
    NOW() + INTERVAL '2 days',
...
SELECT s.survey_id, u.user_id, '{"q1":"Windows","q2":["Work","Development"],"q3":"More shortcuts would help"}', 29, 1, NOW()
FROM surveys s
...
INSERT INTO responses (survey_id, user_id, answer_data, respondent_age, respondent_gender, answered_at)
SELECT s.survey_id, u.user_id, '{"q1":"Mac","q2":["Video editing"],"q3":"Please add more color options"}', 34, 2, NOW()
FROM surveys s
...
INSERT INTO responses (survey_id, user_id, answer_data, respondent_age, respondent_gender, answered_at)
SELECT s.survey_id, NULL, '{"q1":"Linux","q2":["Development"],"q3":"Lightweight is important"}', 41, 1, NOW()
FROM surveys s
WHERE s.question_key = 'jt_20260702_q_pc'
  AND NOT EXISTS (
      ...
      WHERE r.survey_id = s.survey_id
        ...
        AND r.answer_data = '{"q1":"Linux","q2":["Development"],"q3":"Lightweight is important"}'::jsonb
  );

INSERT INTO responses (survey_id, user_id, answer_data, respondent_age, respondent_gender, answered_at)
SELECT s.survey_id, u.user_id, '{"q1":"The explanation is easy to read","q2":"More examples would help"}', 22, 2, NOW()
FROM surveys s
...
INSERT INTO responses (survey_id, user_id, answer_data, respondent_age, respondent_gender, answered_at)
SELECT s.survey_id, NULL, '{"q1":"The flow is a little hard to follow","q2":"Please improve search"}', 38, 1, NOW()
FROM surveys s
WHERE s.question_key = 'jt_20260702_q_free'
  AND NOT EXISTS (
      SELECT 1 FROM responses r
      WHERE r.survey_id = s.survey_id
        AND r.user_id IS NULL
        AND r.answer_data = '{"q1":"The flow is a little hard to follow","q2":"Please improve search"}'::jsonb
  );

INSERT INTO responses (survey_id, user_id, answer_data, respondent_age, respondent_gender, answered_at)
SELECT s.survey_id, u.user_id, '{"q1":"Daily","q2":["Speed","Usability"],"q3":"A dark theme would be nice"}', 27, 1, NOW()
FROM surveys s
...
INSERT INTO responses (survey_id, user_id, answer_data, respondent_age, respondent_gender, answered_at)
SELECT s.survey_id, NULL, '{"q1":"Several times a week","q2":["Notifications"],"q3":"Please make mobile input easier"}', 19, 2, NOW()
FROM surveys s
WHERE s.question_key = 'jt_20260702_q_mix'
  AND NOT EXISTS (
      SELECT 1 FROM responses r
      WHERE r.survey_id = s.survey_id
        AND r.user_id IS NULL
        AND r.answer_data = '{"q1":"Several times a week","q2":["Notifications"],"q3":"Please make mobile input easier"}'::jsonb
  );
